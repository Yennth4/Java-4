<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
      integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!-- Link CSS -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css">
<!-- Link angularJS -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.8.3/angular.min.js"></script>
<!-- Link JS -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js"></script>
<!DOCTYPE html>
<html>
<head>
    <title>JSP - Hello World</title>
</head>
<body>
<table class="table table-hover table-bordered container">
    <h1 style="font-family: 'Times New Roman'; color: black" class="text-center mb-3 mt-3">Hello đây là file bài <a
            href="">Java 4</a> của Hải Yến! Một ngày thật đẹp nhớ</h1>
    <thead>
    <tr>
        <th>Demo</th>
        <th>Lab</th>
        <th>BTVN</th>
    </tr>
    </thead>
    <tbody>
    <tr>
        <td><a href="/ban/hien-thi">Quan ly ban be</a></td>
        <td><a href="/nhan-vien/hien-thi">Quan ly nhan vien</a></td>
        <td><a href="/san-pham/hien-thi2">Quan ly san pham</a></td>
    </tr>
    <tr>
        <td><a href="login">Buoi 2</a></td>
        <td><a href="B1Servlet">Lab 2 - Bai 1</a></td>
        <td><a href="Lab1">Lab 1</a></td>
    </tr>
    <tr>
        <td><a href="InputServlet">Buoi 3</a></td>
        <td><a href="B2Servlet">Lab 2 - Bai 2</a></td>
        <td><a href="B6L2Servlet">BTVN 3</a></td>
    </tr>
    <tr>
        <td><a href="sinh-vien/hien-thi">Buoi 4</a></td>
        <td><a href="EmploybeeServlet">Lab 3 - Bai 1</a></td>
        <td><a href="giang-vien/hien-thi">BTVN 4</a></td>
    </tr>
    <tr>
        <td><a href="/san-pham/view-all">Buoi 7</a></td>
        <td><a href="/movie/hien-thi">Asm1</a></td>
        <td><a href="">BTVN 10</a></td>
    </tr>
    <tr>
        <td><a href="/category/hienthi">Buoi 8</a></td>
        <td><a href="/phone/hien-thi">Lab 4</a></td>
        <td><a href="">BTVN 11</a></td>
    </tr>
    <tr>
        <td><a href="">Buoi 9</a></td>
        <td><a href="/NhanVienServlet/hienThi">Lab 5</a></td>
        <td><a href="">BTVN 12</a></td>
    </tr>
    <tr>
        <td><a href="">Buoi 10</a></td>
        <td><a href="/KhachHangServlet">Lab 6</a></td>
        <td><a href="">BTVN 13</a></td>
    </tr>
    </tbody>
</table>
</body>
</html>
